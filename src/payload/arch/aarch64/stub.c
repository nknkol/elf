...
uintptr_t stub_emit(void *slot, uintptr_t payload_addr, uintptr_t return_addr)
{
	...
	uint32_t branch = make_branch((uintptr_t)slot + 13 * 4, return_addr);
	if (branch)
		code[13] = branch;
	branch = make_branch((uintptr_t)slot + 15 * 4, return_addr);
	if (branch)
		code[15] = branch;
